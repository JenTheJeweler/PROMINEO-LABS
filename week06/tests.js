...
describe('Question 1b: Manage my wallet', () => {
  ...
  //can't declare variables with let/const inside of classes! Use this.
    addMoney(amount) {
     this.money += amount
    }
...
describe('Question 1c: Day of the Week', () => {
  const dayOfTheWeek = (num) => {
    if (typeof num !== 'number' || num < 1 || num > 7) {
      console.log('Err. Something went wrong. Invalid input:', num);
      return 'Err. Something went wrong. Invalid input.';
      //added this to try to do the console.log thing...see below...
    ...
    switch (num) {
      ...
      default:
        if (typeof num === 'number' && num >= 1 && num <= 7) {
          ...
          console.log('Err. Something went wrong. Invalid input.');
          //I'm trying to get it to print the error to the console, here, 
          //whenever anything other than a value is input, but I can't figure it out.  
          return 'Err. Something went wrong. Invalid input.';
        }
      }
    }
   ...
   it('#Should return Monday with input of 1', () => {
    ...
    it('#Should return Err. Something went wrong with input of anything else', () => {
      expect(dayOfTheWeek('bean')).to.equal('Err. Something went wrong');
      expect(console.log('Err. Something went wrong. Invalid input.'));
      //printing it at the end...i give up! 
        });
  });
...
describe('Question 1d: Only wizards shall pass', () => {
  const movieCharacters = [
    ...
    },
    {
      name:'Chris',
      isAWizard: true,
      quote: "Bro i swear I'm a wizard"
...
//"know the value in knowing exactly what you're working with"

    //I understood not any of that.  I mean the whole thing, really.
